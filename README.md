...
<p align="center">
  <a href="https://github.com/marketplace/actions/deploy-to-github-pages">
    <img alt="GitHub Pages Deploy Action Logo" width="200px" src="https://github.com/step-security/github-pages-deploy-action/raw/dev/.github/docs/icon.png">
  </a>
</p>
...
<p align="center">
  <img src="https://github.com/step-security/github-pages-deploy-action/raw/dev/.github/docs/screenshot.png" alt="">
</p>

<p align="center">
Maintenance of this project is made possible by all the <a href="https://github.com/step-security/github-pages-deploy-action/graphs/contributors">contributors</a> and <a href="https://github.com/sponsors/JamesIves">sponsors</a>. If you'd like to sponsor this project and have your avatar or company logo appear below <a href="https://github.com/sponsors/JamesIves">click here</a>. 💖
</p>
...
| `clean-exclude`         | If you need to use `clean` but you'd like to preserve certain files or folders from deletion you can use this option. Matching files are only protected from clean-up; they'll still be updated normally if a newer version exists in your deployment folder. This should contain each pattern as a single line in a multiline string.                                                                                                                                                                                                                                                         | `with` | **No**   |
| `include-github-folder` | A `.github` folder in your deployment source is excluded by default to avoid circular deployment issues. Set this to `true` if you actually want to deploy a `.github` folder from your build output. This does not affect `.git` or `.ssh`, which are always excluded.                                                                                                                                                                                                                                                                                                                        | `with` | **No**   |
...
| `lfs`                   | Enables Git Large File Storage (LFS) support. When set to `true`, the action will initialize LFS in the deployment branch, allowing files tracked by LFS (via a `.gitattributes` file in your deployment folder) to be pushed as LFS objects instead of regular Git objects. This is useful when your deployment includes files that exceed GitHub's 100MB file size limit. Git LFS must be available on the runner; it's pre-installed on GitHub-hosted runners but must be installed separately on self-hosted ones. The deploy will fail if `lfs: true` is set and Git LFS isn't available. | `with` | **No**   |
