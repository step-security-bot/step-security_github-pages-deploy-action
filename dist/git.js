...
export async function init(action) {
    try {
         ...
         */
        try {
            await execute(`git config --global --add safe.directory "${action.workspace}"`, action.workspace, action.silent);
        }
        catch {
        ...
        await execute(`git config user.email "${action.email}"`, action.workspace, action.silent);
        ...
        try {
            if ((process.env.CI && !action.sshKey) || action.isTest) {
                ...
                */
                await execute(`git config --local --unset-all http.https://${action.hostname}/.extraheader`, action.workspace, action.silent);
            }
            ...
            info('Unable to unset previous git config authentication as it may not exist, continuing…');
        }
        try {
        ...
        await execute(`git remote add origin ${action.repositoryPath}`, action.workspace, action.silent);
        info('Git configured… 🔧');
    }
    catch (error) {
        throw new Error(`There was an error initializing the repository: ${suppressSensitiveInformation(extractErrorMessage(error), action)} ❌`, { cause: error });
    }
}
/**
    ...
    try {
        ...
        await execute(`rsync -q -av --checksum --progress ${isMkpathSupported && action.targetFolder ? '--mkpath' : ''} ${escapeShellArg(action.folderPath)}/. ${action.targetFolder
            ...
            ? `--delete ${cleanExcludeFilters} ${!fs.existsSync(`${action.folderPath}/${DefaultExcludedFiles.CNAME}`)
                ...
                : ''} ${!fs.existsSync(`${action.folderPath}/${DefaultExcludedFiles.NOJEKYLL}`)
                ? `--exclude ${DefaultExcludedFiles.NOJEKYLL}`
                : ''}`
            : ''}  --exclude ${DefaultExcludedFiles.SSH} --exclude ${DefaultExcludedFiles.GIT} ${action.includeGithubFolder
            ? ''
            : `--exclude ${DefaultExcludedFiles.GITHUB}`} ${action.folderPath === action.workspace
        ...
        const hasFilesToCommit = action.isTest & TestFlag.HAS_CHANGED_FILES ||
            Boolean((await execute(checkGitStatus, `${action.workspace}/${temporaryDeploymentDirectory}`, true // This output is always silenced due to the large output it creates.
            )).stdout);
        ...
        if (action.tag) {
            ...
            await execute(`git tag ${action.tag}`, `${action.workspace}/${temporaryDeploymentDirectory}`, action.silent);
            info(`Pushing '${action.tag}' tag to repository…`);
            await execute(`git push origin ${action.tag}`, `${action.workspace}/${temporaryDeploymentDirectory}`, action.silent);
            info(`Tag '${action.tag}' created and pushed to the ${action.branch} branch… 🏷️`);
        }
        return Status.SUCCESS;
    }
    catch (error) {
        throw new Error(`The deploy step encountered an error: ${suppressSensitiveInformation(extractErrorMessage(error), action)} ❌`, { cause: error });
    }
    finally {
        ...
        await execute(`git checkout -B ${temporaryDeploymentBranch}`, `${action.workspace}/${temporaryDeploymentDirectory}`, action.silent);
        try {
            await execute(`chmod -R +rw ${temporaryDeploymentDirectory}`, action.workspace, true // Always silent to avoid flooding output on read-only folders
            );
        }
        catch {
            // Silently ignore chmod failures - they are non-critical and often occur with read-only folders
        ...
        if (action.lfs) {
            try {
                // git lfs install --local writes to the shared repo config rather than
                // anything worktree-local, so it's undone here to avoid leaking LFS
                // hooks/filters into the rest of the job once this worktree is gone.
                await execute(`git lfs uninstall --local`, action.workspace, true);
            }
            catch {
                // Non-critical cleanup, ignore failures.
            }
        }
        await execute(`git worktree remove ${temporaryDeploymentDirectory} --force`, action.workspace, action.silent);
