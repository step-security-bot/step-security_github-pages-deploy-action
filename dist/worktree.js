 ...
 */
export async function generateWorktree(action, worktreedir, branchExists) {
    try {
        ...
        try {
            await execute(checkout.toString(), `${action.workspace}/${worktreedir}`, action.silent);
        }
        catch (error) {
            if (!action.silent) {
                console.error(error);
            }
            info('Error encountered while checking out branch. Attempting to continue with a new branch name.');
        ...
        if (!branchExists) {
            ...
            await execute('git reset --hard', `${action.workspace}/${worktreedir}`, action.silent);
            if (!action.singleCommit) {
                // New history isn't singleCommit, create empty initial commit
                await execute(`git commit --no-verify --allow-empty -m "Initial ${branchName} commit"`, `${action.workspace}/${worktreedir}`, action.silent);
            }
        }
        /**
         ...
         */
        try {
            await execute(`git config --global --add safe.directory "${action.workspace}/${worktreedir}"`, action.workspace, action.silent);
        }
        catch {
        ...
        if (action.lfs) {
            ...
            await execute(`git lfs install --local`, `${action.workspace}/${worktreedir}`, action.silent);
        }
    }
    catch (error) {
        throw new Error(`There was an error creating the worktree: ${suppressSensitiveInformation(extractErrorMessage(error), action)} ❌`, { cause: error });
    }
}
